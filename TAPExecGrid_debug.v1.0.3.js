...
var _renderStatistics = function (data, index) {
	...
	if (sumPts != 0 || (sucPts+falPts+blkPts+wrkPts) != 0) {
		...
		if (data.status == "WORKING") {
			tags += "<div style='float:left;display:block;text-align:center;color:#999999;font-style:italic;width:"
			+ maxWidth 
			+"px;'>Installing or Configuring environment ...</div>";
		}
		else if (data.status == "STOPPED" || data.status == "NOTATTEMPTED") {
			tags += "<div style='float:left;display:block;text-align:center;color:#999999;font-style:italic;width:"
			+ maxWidth 
			+"px;'>No test scripts have been executed yet...</div>";
		}
		else if (data.status == "SUCCESSFUL" || data.status == "FAILED") {
			tags += "<div style='float:left;display:block;text-align:center;color:#999999;font-style:italic;width:"
			+ maxWidth 
			+"px;'>No test scripts have been executed yet...</div>";
		}
		else {//it is actual a error state.
			tags += "<div style='float:left;display:block;text-align:center;color:#FE0000;font-style:italic;width:"
			+ maxWidth 
			+"px;'>ER points are not calculated correctly...</div>";
		}*/
	}
...
define ([
	...
	return declare ("TAPExecGrid", [_WidgetBase, _TemplatedMixin], {
		...
		_selectGridRow: function selectGridRow_TAPGrid (row) {
			...
			if (this.bindingData._objectPath == "local.myExecutionOverviews") {
				for (var i=0; i<items.length; i++) {
